...
if "start_date_input" not in st.session_state:
    # 기본값을 '오늘'이 아닌 3일 후로 잡는다. 당일(same-day) 항공권 검색은
    # Google Flights/SerpApi 쪽에서 결과가 불안정하게 비는 사례가 보고돼 있어,
    # 실제 여행 계획과도 더 맞는 '며칠 뒤' 출발을 기본값으로 둔다.
    st.session_state["start_date_input"] = date.today() + timedelta(days=3)
if "end_date_input" not in st.session_state:
    st.session_state["end_date_input"] = date.today() + timedelta(days=10)
        ...
        if origin_err or dest_err:
            ...
            with st.spinner("데이터 통신 및 최적화 계산 중..."):
                ...
                if not flights or not hotels:
                    ...
                    st.caption("ℹ️ Google Flights/SerpApi 쪽에서 일시적으로 빈 결과를 주는 경우가 보고돼 있습니다. 아래 버튼으로 캐시를 지우고 바로 다시 조회해볼 수 있습니다.")
                    if st.button("🔄 다시 시도 (캐시 지우고 재조회)", key="retry_search_button"):
                        get_cached_api_data.clear()
                        st.rerun()
                else:
